        ...
        std::cout<<bool(highPriority1);//1 correct.
         ...
         auto ep1=static_cast<float>(calcEpsilon());
          ...
          auto ep2=b[5];
            ...
            using std::bitset;
            using std::size_t;
           bitset<17>::reference (bitset<17>::*ep4)(size_t) ;
             bool (bitset<17>::*ep5)(size_t)const ;
           ep4=&bitset<17>::operator[];
           ep5=&bitset<17>::operator[];
           std::cout<<'\n'<<type_id_with_cvr<decltype(ep4)>().pretty_name()<<"\n";
            //std::bitset<17ul>::reference (std::bitset<17ul>::*)(unsigned long)

               std::cout<<'\n'<<type_id_with_cvr<decltype(ep5)>().pretty_name()<<"\n";
                //bool (std::bitset<17ul>::*)(unsigned long) const
